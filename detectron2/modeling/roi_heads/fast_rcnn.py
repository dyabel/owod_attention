...
class FastRCNNOutputLayers(nn.Module):
    ...
    def clstr_loss_l2_cdist(self, input_features, proposals):
        ...
        Compute loss: if belonging to a class -self.margin likelihood should be higher
                      else -> lower
    ...
    def get_clustering_loss(self, input_features, proposals):
        ...
        storage = get_event_storage()
        c_loss = 0
        if storage.iter >= self.clustering_start_iter and self.flag:
            self.flag = False
    ...
    def memory_loss(self, input_features,proposals,pred_class_logits):
        ...
        V = self.attention_value(input_one_for_value).reshape((self.num_classes+1)*self.key_num_per_cls,in_features_cur.shape[1])
